...
#load("~/Desktop/HNF1A/all_cells_clean.rda")
#load("~/Desktop/HNF1A/label_clean.rda")

lbl.data <- lbl.data # separate label from numeric data
num.data <- all.data

# Top 100 genes from each cell type identified by tSNE using primary pancreatic cells
markers <- read.table("Cell_markers.txt", sep="\t", row.names=1, header = T, stringsAsFactors = F) 
markers <- markers[,1] #100 genes specific for each cell type identified in primary pancreatic cells
...
# high resolution will give you more subgroups, but we want this to happen. 
# we will regroup the subgroups based on the class of cells they are.
# This is to separate the smaller populations (delta, exocrine cells) 
# from the other bigger cluster.
...
FeaturePlot(seu.data, features.plot = "SST", cols.use = c("grey", "blue"), reduction.use = "tsne", no.legend = F, pt.size = 0.6) # delta
FeaturePlot(seu.data, features.plot = "PPY", cols.use = c("grey", "blue"), reduction.use = "tsne", no.legend = F, pt.size = 0.6) # gamma
FeaturePlot(seu.data, features.plot = "KRT19", cols.use = c("grey", "blue"), reduction.use = "tsne", no.legend = F, pt.size = 0.6) #ductal
FeaturePlot(seu.data, features.plot = "COL1A2", cols.use = c("grey", "blue"), reduction.use = "tsne", no.legend = F, pt.size = 0.6) #pancreatic stellate cells
FeaturePlot(seu.data, features.plot = "GHRL", cols.use = c("grey", "blue"), reduction.use = "tsne", no.legend = F, pt.size = 0.6)
FeaturePlot(seu.data, features.plot = "MUC1", cols.use = c("grey", "blue"), reduction.use = "tsne", no.legend = F, pt.size = 0.6)
...
### incorporate a few markers
temp <- seu.data.sub@scale.data
temp <- temp[c("INS","PDX1","MAFA","NKX6-1","SYP","CGA"),]

temp_bi <- apply(temp,2,function(x) ifelse(x>0,1,0))
pr<-prcomp(temp_bi, scale = F, center = F)
tsne <- Rtsne(t(temp_bi), theta=0,perplexity=30)
plot(pr$x[,1],pr$x[,2],pch=20,cex=0.1)

temp.marker <- temp[c("INS","PDX1","MAFA","NKX6-1","SYP","CGA"),]

temp0 <- colnames(seu.data.sub.0)
temp2 <- colnames(seu.data.sub)


### plot the mean distribution of multiple targets
pdf("mean z-score combined.pdf")
hist(apply(temp[,2671:5803],2,sum)/6,breaks = 20,main = "mean zscore distribution in cluster 0 and cluster 2",xlab = "mean zscore")
dev.off()

pdf("mean z-score distribution.pdf",width =8,height=8)
hist(apply(temp[,2671:5803],2,mean),freq=T,breaks=40,col=rgb(0,0,1,1/4),xlim = c(-1.5,3),ylim = c(0,400), 
     xlab = "mean z-score",main = "mean z-score of cluster0 and cluster2",cex.lab=1.5)
hist(apply(temp[,1:2670],2,mean),breaks=40,col=rgb(1,0,0,1/4),freq=T,add=T)

legend("topright", c("cluster 0", "cluster 2"), fill=c(rgb(0,0,1,1/4), rgb(1,0,0,1/4)),cex=1.5)
dev.off()
...
### plot the percentage of different cells
cell_stat <- read.table("cell_stat.txt", sep="\t", row.names=1, header = T, stringsAsFactors = F)
cell_stat <- t(cell_stat[,1:3])
cell_stat_2 <- apply(cell_stat,1,function(x) round(x*x[4]))[1:3,]
cell_stat_2 <- cell_stat_2[,1:5]

data_percentage <- apply(cell_stat_2, 1, function(x){x/sum(x)})


library(RColorBrewer)
 
pdf("barplot.pdf",height = 10,width=12)
col = brewer.pal(3, "Pastel2")
par(mar=c(4,4,4,4))
#barplot(cell_stat, col=col,horiz=T,xlab = "",las=2,cex.names=1.3,legend.text =rownames(cell_stat),
        #args.legend = list(x ='right', bty='n', inset=c(-0.3,0), xpd = T))
barplot(t(data_percentage), col=col,beside=T,legend=colnames(data_percentage),horiz=T,las=2,xlab = "",
        cex.names=1.5,args.legend = list(x ='bottomright', bty='n',cex = 1.5))
dev.off()
